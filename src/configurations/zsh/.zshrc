...
__conda_setup="$('/usr/local/share/c3/conda/bin/conda' 'shell.bash' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
    if [ -f "/usr/local/share/c3/conda/etc/profile.d/conda.sh" ]; then
        . "/usr/local/share/c3/conda/etc/profile.d/conda.sh"
    else
        export PATH="/usr/local/share/c3/conda/bin:$PATH"
    fi
fi
unset __conda_setup
# <<< conda initialize <<<
