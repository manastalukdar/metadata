...
for list in "${LISTS[@]}"; do
    generated="$(build "$list")"
    target="$PKGS/$list.txt"
    if ((CHECK)); then
        if ! diff -q "$target" "$generated" >/dev/null 2>&1; then
            echo "STALE: pkgs/$list.txt does not match src/software-to-install/" >&2
...
if [[ -s "$TMP/untagged" ]]; then
    echo ""
    echo "Untagged Mandatory entries ($(wc -l < "$TMP/untagged")) — no package manager will install these."
    echo "Add a manager tag, or \`manual\` to acknowledge it needs a hand install:"
    cat "$TMP/untagged"
fi
